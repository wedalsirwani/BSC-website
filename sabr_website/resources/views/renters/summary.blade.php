        ...
        <div class="container">
            ...
            <ul class="text-right">
                ...
                <div class="row">
                    ...
                    <p class="h4 text-right mr-2 d-inline-block">{{$renter->name}}</p>
                </div>
                <div class="row">
                    <p class="h4 font-weight-bold text-right mr-5 d-inline-block">رقم الهوية الوطنية / رقم هوية مقيم:</p>
                    <p class="h4 text-right mr-2 d-inline-block">{{$renter->id_number}}</p>
                </div>
                <div class="row">
                    <p class="h4 font-weight-bold text-right mr-5 d-inline-block">رقم الجوال:</p>
                    <p class="h4 text-right mr-2 d-inline-block">{{$renter->phone_number}}</p>
                </div>
                <?php $total_amount=0; $total_payable=0; $total_paied=0; $total_reminig=0;$sum_benefits=0;?>
                @if($renter->contracts->count() >0)
                @foreach($renter->contracts as $contract)
                    <?php
                        $total_amount+=$contract->rent_amount;
                        $total_payable+=$contract->payable_this_contract();
                        $total_paied+=($contract->rent_amount-$contract->remaining());
                        $total_reminig+=$contract->remaining();
                    ?>
                ...
                <div class="row mb-3">
                        <div class="table-responsive text-center">
                            <p class="h4 text-center w-100 font-weight-bold">ملخص كشف الحساب</p>
                            <table class="table table-striped">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td>
                                            {{number_format($total_paied,2)}}
                                        </td>
                                        <td>
                                        {{number_format($sum_benefits)}}
                                        </td>
                                        <td>
                                            {{number_format($total_reminig+$sum_benefits,2)}}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
